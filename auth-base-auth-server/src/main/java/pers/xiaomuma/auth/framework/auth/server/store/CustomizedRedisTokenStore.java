...
import pers.xiaomuma.auth.framework.auth.server.store.serialize.JsonSerializationStrategy;
...
public class CustomizedRedisTokenStore implements CustomTokenStore {
	...
	private RedisTokenStoreSerializationStrategy serializationStrategy = new JsonSerializationStrategy();

	private String prefix = "";
	private Method redisConnectionSet_2_0;

	public CustomizedRedisTokenStore(RedisConnectionFactory connectionFactory) {
		this.connectionFactory = connectionFactory;
		if (springDataRedis_2_0) {
			this.loadRedisConnectionMethods_2_0();
		}
		redisTemplate = initialRedisTemplate();
	}
	...
	private RedisTemplate<?, ?> initialRedisTemplate() {
		RedisTemplate<?, ?> redisTemplate = new RedisTemplate<>();
		redisTemplate.setConnectionFactory(connectionFactory);
		Jackson2JsonRedisSerializer<Object> jackson2JsonRedisSerializer = new Jackson2JsonRedisSerializer<>(Object.class);
		jackson2JsonRedisSerializer.setObjectMapper(JsonUtils.OBJECT_MAPPER);
	...
	@Override
	public FindKeysResult findKeysForPage(Integer pageNum, Integer pageSize) {
		String keyPrefix = prefix + ACCESS;
		String patternKey = keyPrefix + "*";
		...
		while (cursor.hasNext()) {
			...
			if (tmpIndex >= startIndex && tmpIndex < end) {
				String accessKey = StrUtil.subAfter(object.toString(), keyPrefix, true);
		...
		return new FindKeysResult(result, tmpIndex);
	}

	private CustomTokenStoreException operationNotSupported() {
		return new CustomTokenStoreException("This operation is not supported.");
	}
